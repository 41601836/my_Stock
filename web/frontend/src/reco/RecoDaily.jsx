  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: '快照天数',     value: dates.length || 0, unit: '天', color: 'text-amber-400' },
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center h-48 text-gray-500 gap-2">
          ...
          <p className="text-sm">暂无每日推荐快照数据</p>
        </div>
      ) : (
                      ...
                      <table className="w-full text-xs border-collapse">
                        <thead>
                          <tr className="bg-[#080F1C]">
                            {['排名','股票','因子分','筹码胜率','大单净流入','上榜后5日超额'].map(h => (
                          ...
                          {rows.map(r => (
                            <tr key={r.ts_code} className="border-t border-[#1A2840]/40 hover:bg-[#0D1B2E]/40 transition-colors">
                              ...
                              <td className="py-2 px-3 text-center">
                                {r.winner_rate ? <span className="text-sky-400 font-mono">{r.winner_rate.toFixed(1)}%</span> : <span className="text-gray-600 font-mono">—</span>}
                              </td>
                              <td className="py-2 px-3 text-center">
                              ...
                              <td className="py-2 px-3"><PctChg value={r.alpha_5d_pct} /></td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
